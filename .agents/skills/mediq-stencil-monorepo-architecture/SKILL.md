---
name: mediq-stencil-monorepo-architecture
description: |
  ...
  or cross-framework plans for React/Angular/Vue. Triggers on: mediq components, stencil monorepo,
...
# Mediq Stencil Monorepo Architecture Skill
...
- `packages/stencil-library` -> `@mediq/stencil-library`
- `packages/react-library` -> `@mediq/react-library`
...
If missing, TypeScript in `@mediq/react-library` may fail to resolve generated imports like:
`@mediq/stencil-library/dist/components/<component>.js`.
...
3. Keep dependency on:
   ...
   - `@mediq/stencil-library` via `workspace:*`

## Consumer App Rules (React Demo)

In `apps/react-demo`:

1. Depend on `@mediq/react-library` via `workspace:*`.
2. Import wrappers directly, e.g. `import { MyComponent } from '@mediq/react-library'`.
